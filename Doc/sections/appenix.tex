...
\begin{equation}
    \rho\left(\frac{\alpha}{1 + \alpha} \hat A \right) = \frac{\alpha}{1 + \alpha} \rho\left( \hat A \right) <  \frac{\alpha}{1 + \alpha} < 1
\end{equation}
...
\begin{equation*}
    H^{(l + 1)} = \sigma(K^{-1}_ H^{(l)} W^{(l)}) \approx \sigma\left(\frac{1}{1 + \alpha}\sum_{k = 0}^{\infty}\left(\frac{\alpha}{1 + \alpha}\right)^{k} \hat A^k H^{(l)} W^{(l)}\right)
\end{equation*}

where $\alpha$ controlls the effective propagation length.
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \For{$b = 0, \dots ,B - 1$}
        \State $H_{b + 1} \gets \texttt{MPSSM}_b(H_b,S)$
        \State $H_{b + 1} \gets \texttt{GraphNorm}(\texttt{Dropout}_p(H_{b}))$
        \EndFor
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \State $z \gets \texttt{solve\_triangular}(L, y, \text{upper} = T)$
        \State \Return $z$
    \end{algorithmic}
\end{algorithm}
...
passing steps instead of 4 to have a close parameter count to ours.
...
We used identical hyperparameters to \ref{tbl:hyperparams_mpssmpcg} in our
long-range propagation evalutaion apart from a learning rate of $10^{-4}$ and
we trained for 30 epochs instead of 50.
